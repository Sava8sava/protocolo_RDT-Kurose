...
// Windows: g++ client_cross_platform.cpp -o client.exe -lws2_32 -lcrypto
// Linux:   g++ client_cross_platform.cpp -o client -lcrypto
// macOS:   g++ client_cross_platform.cpp -o client -lcrypto
...
void thread_connect(socket_t sockfd, char buffer[], int bufferlen) {
...
int main()
{
    ...
    if (res != NO_ERROR) {
        PRINT_ERROR(L"Erro ao iniciar WinSock\n");
        return 1;
    }
#endif
    ...
    if (Sapi == INVALID_SOCKET_VALUE) {
        PRINT_ERROR("Erro ao criar socket: %d\n", GET_ERROR);
#ifdef _WIN32
        WSACleanup();
#endif
        return 1;
    ...
    if (Sapi == INVALID_SOCKET_VALUE) {
        PRINT_ERROR("Erro ao criar segundo socket: %d\n", GET_ERROR);
#ifdef _WIN32
        WSACleanup();
#endif
        return 1;
    ...
    int res = bind(Sapi,(sockaddr *) &clientaddr, sizeof(clientaddr));
    if (res == SOCKET_ERROR_CODE)
    {
        PRINT_ERROR("Ligacao do socket falhou com erro %d\n", GET_ERROR);
        CLOSE_SOCKET(Sapi);
#ifdef _WIN32
        WSACleanup();
#endif
    ...
    {
        PRINT_ERROR("Bind finalizada com sucesso\n");
    }
    ...
    PRINT_ERROR("Recebendo datagramas do servidor...\n");
    while (true)
    {
        res = recvfrom(Sapi, (char *)buffer, bufferlen, 0, (sockaddr *) &sendback, &sendbackSize);
        if (res == SOCKET_ERROR_CODE)
        {
            PRINT_ERROR("Recvfrom falhou com erro %d\n", GET_ERROR);
        }
        for (int i = 0; i < res; i++)
        {
            printf("%c", buffer[i]);
    ...
    PRINT_ERROR("Fim do recebimento. Fechando socket\n");
    res = CLOSE_SOCKET(Sapi);
    if (res == SOCKET_ERROR_CODE) {
        PRINT_ERROR("Fechamento falhou com erro %d\n", GET_ERROR);
        return 1;
    }
